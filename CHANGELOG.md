# Storico delle edizioni

Ogni push su `main` produce una nuova edizione del ricettario, con il PDF rigenerato.

## v0.7.0 — 2026-08-19

### Modifiche al ricettario

- feat: modificare una ricetta dal sito, con note firmate

## v0.6.0 — 2026-08-17

### Modifiche al ricettario

- feat: aggiungere un contributo dal sito, senza aprire l'editor

## v0.5.0 — 2026-08-14

### Modifiche al ricettario

- feat: contributi firmati e versione di ogni ricetta

## v0.4.0 — 2026-08-14

### Modifiche al ricettario

- feat: immagini segnaposto con la scritta "Foto non ufficiale"

## v0.3.0 — 2026-08-11

### Ricette aggiunte

- Pasta alla carbonara

### Ricette rimosse

- bruschette-al-pomodoro
- friarielli-ripassati
- limoncello
- pasta-e-patate-con-la-provola
- pastiera-napoletana
- pesto-alla-genovese
- pizza-in-teglia
- polpette-al-sugo
- ragu-della-domenica

### Modifiche al ricettario

- feat: prime ricette vere e foto per ogni passaggio

## v0.2.0 — 2026-08-11

### Ricette aggiunte

- Bruschette al pomodoro e basilico
- Friarielli ripassati
- Limoncello
- Pasta e patate con la provola
- Pastiera napoletana
- Pesto di basilico
- Pizza in teglia a lunga lievitazione
- Polpette al sugo
- Ragù della domenica

### Modifiche al ricettario

- feat: ricettario di famiglia con sito navigabile e libro PDF versionato
- Initial commit
